...
- Полный DRF-слой оправдан с мобильным клиентом или публичным API. Для
  production-диагностики используются небольшие версионированные Django JSON
  endpoints: публичное состояние приложения, сессионный список собственных
  push-устройств и безопасный self-test с CSRF. Endpoint и ключи подписок API
  не возвращает.
