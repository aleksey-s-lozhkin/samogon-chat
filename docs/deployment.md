...
AUDIO_MESSAGE_MAX_SIZE=10485760
...
Для текущего профиля на VM с Ollama достаточно загрузить базовую модель:

```bash
ollama pull qwen3:8b
```

Системный промпт хранится в `chat/services/prompts/semen.txt` и передаётся
приложением, поэтому отдельный Ollama-профиль не требуется. Production-модель
выбирается переменной `OLLAMA_MODEL`.
...
После деплоя миграция добавляет в каталог несколько точных строк PEP 20. Чтобы
подготовить модельные варианты, поставьте фоновые задания из web-контейнера:

```bash
cd /srv/compose/samogon
docker compose exec samogon-web python manage.py generate_atmosphere_lines --count 10
```

Worker сохранит результаты как черновики. Откройте в Django Admin раздел
«Атмосферные строки», отметьте подходящие варианты и примените действие
«Одобрить выбранные строки». Только активные одобренные строки появляются в
шапке чата. Публикация не требует перезапуска контейнеров.

### Режим регистрации и лимиты

Для открытой беты задайте `REGISTRATION_OPEN=1` в production `samogon.env`
и пересоздайте контейнеры через штатный deploy. Поле приглашения скрывается,
сохранённый `REGISTRATION_INVITE_CODE` не проверяется. Turnstile, проверка
пароля, уникальность email и лимиты запросов продолжают действовать.
После deploy проверьте регистрацию обычного аккаунта без кода приглашения.
Для возврата прежнего режима задайте `REGISTRATION_OPEN=0`.

При `REGISTRATION_OPEN=0` и `DEBUG=0` пустой `REGISTRATION_INVITE_CODE` отключает регистрацию. Тестер
...
Перед заменой контейнеров workflow запоминает точное имя образа одновременно
работающих web и worker. Если новый web или worker не проходит health check,
журналы неудачной версии сохраняются в Actions, оба сервиса возвращаются на
предыдущий образ, workflow ждёт запуска прежнего web и обновляет Nginx. Запуск
при этом остаётся неуспешным, чтобы ошибка не была скрыта зелёным статусом.

Автоматический откат возвращает код и контейнеры, но не откатывает миграции
PostgreSQL. До release candidate миграции должны оставаться совместимыми с
предыдущей версией приложения: сначала добавление новых таблиц и полей, и лишь
в отдельном последующем релизе — удаление старых. Несовместимые миграции требуют
запланированного окна и проверенного восстановления резервной копии.
...
WebSocket, личное сообщение, переключение между закрытыми беседами, управление
собственной беседой, выход приглашённого участника, счётчик непрочитанного и
ответ `@Семён`.
...
на настроенном хосте приложения. Перед загрузкой образа deploy требует не
меньше 5 ГБ свободного места, а после успешного запуска сохраняет текущий и
предыдущий rollback-образ и удаляет более старые SHA. Web-контейнер ждёт
доступности PostgreSQL до 60 секунд перед миграциями, поэтому краткая задержка
Docker DNS не создаёт бесконечный цикл перезапуска.
...
Для быстрой диагностики Push во время беты откройте профиль →
«Диагностика уведомлений» → «Скопировать диагностику». Отчёт содержит
состояние браузера, service worker и подписки, но не cookies, ключи подписки
или тексты сообщений. Для остальных дефектов сохраняйте время, устройство,
шаги воспроизведения и сопоставляйте их с журналами web/worker.
