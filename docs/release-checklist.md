...
## При включении открытой регистрации

После deploy изменения регистрации задайте `REGISTRATION_OPEN=1` в
production env-файле и примените его штатным deploy. Простого `docker restart`
недостаточно для обновления переменных окружения контейнера.

1. В приватном окне откройте регистрацию: поля приглашения быть не должно.
2. Зарегистрируйте обычный тестовый аккаунт без приглашения и проверьте вход.
3. Проверьте ошибки обязательных полей, слабого пароля и занятого email.
4. Подтвердите работу production Turnstile; лимиты регистрации остаются включены.
5. В профиле раскройте «Диагностика уведомлений» и скопируйте отчёт.
6. Проверьте свежие логи web/worker на ошибки. Предыдущий production smoke
   не подтверждает работу нового образа — выполните публичный smoke для него.
